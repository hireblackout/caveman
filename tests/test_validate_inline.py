...
from scripts.validate import extract_inline_codes, validate_inline_codes, validate, ValidationResult
...
class TestValidateInlineCodes(unittest.TestCase):
    ...
    def test_added_duplicate(self):
        result = ValidationResult()
        validate_inline_codes("use `cmd` here", "use `cmd` `cmd` here", result)
        self.assertTrue(result.is_valid)
        self.assertIn("Inline code added", result.warnings[0])
